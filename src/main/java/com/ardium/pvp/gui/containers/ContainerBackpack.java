package com.ardium.pvp.gui.containers;

import com.ardium.pvp.inventory.InventoryBackpack;
import com.ardium.pvp.inventory.SlotBackpack;
...
public class ContainerBackpack extends Container {
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index < this.invBackpack.getSizeInventory()) {
				if (!this.mergeItemStack(itemstack1, this.invBackpack.getSizeInventory(), this.inventorySlots.size(),
						true))
					return null;
	...
	public ItemStack slotClick(int slotIndex, int buttonPressed, int flag, EntityPlayer player) {
		...
		if (flag == 2 && buttonPressed == player.inventory.currentItem)
			return null;
		if (slotIndex - this.invBackpack.getSizeInventory() - 27 == player.inventory.currentItem)
			return null;
